...
set.seed(1)
data_1 = bdgraph.sim(n = 500, p = 26, graph = "random", prob = 0.2)
for (i in 1:500){
  for (j in class_1){
...
set.seed(2)
data_2 = bdgraph.sim(n = 500, p = 24, graph = "random", prob = 0.8)
for (i in 1:500){
  for (j in class_2){
...
set.seed(3)
data_3 = bdgraph.sim(n = 500, p = 26, graph = "hub")
for (i in 1:500){
  for (j in class_3){
...
set.seed(4) 
data_4 = bdgraph.sim(n = 500, p = 24, graph = "scale-free")
for (i in 1:500){
  for (j in class_4){
...
output$p_links
p_link = output$p_links +t(output$p_links)+diag(100)
